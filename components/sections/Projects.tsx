...
function ProjectCard({ project, index, visible, onImageClick }: {
  project: Project; index: number; visible: boolean; onImageClick: (img: string) => void;
}) {
  ...
  return (
    <div className="card card-hover flex flex-col overflow-hidden"
      ...
      {project.image && !imgError ? (
        <div className="relative w-full overflow-hidden cursor-zoom-in"
      ...
      {/* Gallery thumbnails — shown when project has multiple images */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {project.featured && (
            <span className="text-xs font-mono px-2 py-0.5 rounded-full"
              style={{ backgroundColor: "rgba(240,165,0,0.1)", border: "1px solid rgba(240,165,0,0.3)", color: "#f0a500" }}>Featured</span>
          )}
          <span className="text-xs font-mono px-2 py-0.5 rounded-full"
